...
public class HeartContainerItem extends Item{
	...
	@Override public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn){
		tooltip.add(new TranslationTextComponent("tooltip.heart_container.1",
				new TranslationTextComponent("tooltip.heart_container.1.hearts").setStyle(new Style().setColor(TextFormatting.YELLOW)),
				new StringTextComponent(Integer.toString(PlayerMovement.MAX_HEART_CONTAINERS)).setStyle(new Style().setColor(TextFormatting.YELLOW))

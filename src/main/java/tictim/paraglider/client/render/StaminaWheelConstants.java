...
public final class StaminaWheelConstants {
	...
	public static final int IDLE = ARGB.color(0, 223, 83);
	public static final int GLOW = ARGB.color(255, 255, 255);
	public static final int EVIL_GLOW = ARGB.color(248, 223, 223);
	public static final int TRANSPARENT = ARGB.color(0, 255, 255, 255);
	...
	public static int getGlowAndFadeColor(long time) {
		...
		else if (time < GLOW_FADE_END)
			return ARGB.lerp((float)(time - GLOW_FADE_START) / GLOW_FADE_DURATION, GLOW, IDLE);
		else if (time < FADE_START) return IDLE;
		else if (time < FADE_END) return ARGB.lerp((float)(time - FADE_START) / FADE_DURATION, IDLE, TRANSPARENT);

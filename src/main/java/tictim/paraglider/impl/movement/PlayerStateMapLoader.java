...
public final class PlayerStateMapLoader {
	...
	private static @NotNull Map<Identifier, State> gatherStates(List<PluginInstance<MovementPlugin>> plugins) {
		...
		Map<Identifier, State> states = new Object2ObjectOpenHashMap<>();
	...
	private static void gatherStateModifications(List<PluginInstance<MovementPlugin>> plugins, Map<Identifier, State> states) {
		List<PluginAction<MovementPlugin, ChangeDefaultStaminaDelta>> staminaDeltaChanges = new ArrayList<>();
		Object2IntMap<Identifier> idToCountMap = new Object2IntArrayMap<>();

		Map<Identifier, Set<Identifier>> flagAdditions = new Object2ObjectOpenHashMap<>();
		Map<Identifier, Set<Identifier>> flagRemovals = new Object2ObjectOpenHashMap<>();
	...
	private static void gatherStateConnections(List<PluginInstance<MovementPlugin>> plugins, Map<Identifier, State> states) {
		...
		Map<Identifier, List<Disconnect>> disconnections = new Object2ObjectLinkedOpenHashMap<>();
		...
		for (var e : fallbacks.entrySet()) {
			...
			@Nullable Identifier fallback = list.get(0).action().fallback();
			boolean same = true;
			for (int i = 1; i < list.size(); i++) {
